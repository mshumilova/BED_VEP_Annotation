...
def docker_pull():
    ...
    cmd3 = 'docker pull ensemblorg/ensembl-vep:latest'
    print(cmd1)
    os.system(cmd1)
    print(cmd2)
    os.system(cmd2)
...
def vep_annotation():
    ...
    for vcf_file_name in inp_file_names_list:
        cmd=f"sudo docker run --platform=linux/amd64 -t -i -v {parent_dir}:/opt/vep/scr/.vep:Z ensemblorg/ensembl-vep ./vep --cache --offline --format vcf --vcf --database --force_overwrite \
        --dir_cache /opt/vep/scr/.vep/{vep}/ \
        --dir_plugins /opt/vep/scr/.vep/{vep}/{plugins}/ \
        --input_file /opt/vep/scr/.vep/{data}/{inp}/{vcf_file_name} \
        --fasta /opt/vep/scr/.vep/{ref}/{ref_file_name} \
        --output_file /opt/vep/scr/.vep/{data}/{annotated_vcf}/{vcf_file_name} \
        --hgvs \
        --symbol \
        --polyphen b \
        --pubmed \
        ...
        --pick_allele \
        --assembly {GRCh} \
        --custom /opt/vep/scr/.vep/{vep}/{plugins}/{gnomad}/{gnomad_exomes_file_name},gnomADe,vcf,exact,0,AF_NFE \
        --custom /opt/vep/scr/.vep/{vep}/{plugins}/{gnomad}/{gnomad_genomes_file_name},gnomADg,vcf,exact,0,AF_NFE \
        --custom /opt/vep/src/.vep/{vep}/{plugins}/{clinvar}/{clinvar_file_name},ClinVar,vcf,exact,0,CLNSIG,CLNREVSTAT,CLNDN,CLN,CLNACC,CLNDSDB,CLNDSDBID \
        --plugin CADD,/opt/scr/vep/.vep/{vep}/{plugins}/{cadd}/{cadd_whole_file_name}"
        print(cmd)
        os.system(cmd)
